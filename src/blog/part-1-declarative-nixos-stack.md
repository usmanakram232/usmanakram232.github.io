...
Every Linux developer has a `~/dotfiles` repo. Mine was, for years, a pile of shell scripts: `createlinks.sh` to symlink configs, `init.sh` to install packages, and a growing mess of `.bashrc`/`.zshrc`/`.vimrc` files that got messier every time I touched them.

The dotfiles themselves weren't the problem. The gap between what I intended and what was actually on the machine was. A package would upgrade and my carefully tuned `alacritty.yml` would stop working. I'd install a tool on one laptop and forget it existed on another. `~/.config/` looked different on every machine, and "reproducible setup" really meant re-reading my own README and hoping the `dnf install` commands still worked.
...
A NixOS config this complete pays off in a few concrete ways. `git clone` plus `nixos-rebuild switch` gets me an identical machine, no manual steps in between. Every generation is a boot option, so a broken upgrade is a reboot away from being undone, not a debugging session. Every installed package and every config change lives in version control, which means the history of the machine is just `git log`. The same modules run on Fedora through `home-manager switch --flake .#usman`, so I can migrate incrementally instead of committing to a full reinstall. And because there's no ambient state, the machine is exactly what the config says — nothing accumulated, nothing forgotten.
