...
Every dotfile repo has entropy. Packages accumulate, configs get stale, security defaults get overridden by convenience and forgotten. After two years of iterating the NixOS desktop, then porting it to macOS, I ran a systematic audit of every layer of the setup.

This post covers the audit methodology, the threat model I used, and the concrete findings. It's structured so you can run the same audit on your own setup.
...
This maps to an eight-layer defence-in-depth stack. I audited each layer independently.
