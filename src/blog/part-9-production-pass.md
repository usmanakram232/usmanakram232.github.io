...
There's a predictable pattern with cross-platform configs: the first platform gets hand-crafted care. The second gets a port. Ports accumulate bugs. You don't find them until the feature is actually used.
...
The original macOS work config in `mac-extras.nix` was a faithful port of `work.nix`. Both files defined the same seven SSM tunnel names, the same hosts, the same AWS region. One drove `systemd.user.services` on Linux. The other drove `launchd.agents` on macOS.
...
**`https://name.localhost` without root.** The portless proxy runs on port 8443 instead of 443, because macOS reserves port 443 for processes running as root. On Linux, `CAP_NET_BIND_SERVICE` lets portless bind 443 without root. That capability doesn't exist on macOS.
...
AI agents shouldn't hold long-lived AWS credentials. They should assume a minimal role, get a one-hour token, and have the token expire. This script creates that role:
