...
The setup is modeled after Andrej Karpathy's ["LLM Wiki" gist](https://gist.github.com/karpathy/442a6bf555914893e9891c11519de94f). The premise is simple: you feed raw input; the LLM synthesizes it into a wiki. You never manually edit the wiki. You capture, and periodically you compile.

It flips where the effort goes. Traditional note-taking asks you to maintain structure as you go. Here, you maintain nothing. You just capture. Structure comes out of compilation.
...
This matters for a few reasons. Work concepts (service names, API quirks, sprint decisions) don't belong mixed into general knowledge. If I ever share the wiki, I can share just `wiki/concepts/` without leaking anything work-specific. And general concepts age slowly while work knowledge can go stale in days. They need different treatment.
...
The wiki is always at higher quality than the raw captures. Because the LLM synthesizes rather than dumps, reading it is useful. Raw notes are noisy and context-heavy. Wiki articles are clean and referenced.
