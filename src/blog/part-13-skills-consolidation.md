...
Back in Part 2 I wrote that this setup managed 93 agent skills, declaratively, via nix. That was January. By August, an audit of `~/.agents/skills/` found 202 entries, and only 4 of them were real nix-store symlinks. Another 116 were imperative symlinks planted by `add_skill`, `npx skills`, or a restore script — working, but never declared anywhere. The remaining 82 were plain real directories: not tracked in git, not referenced in `catalog.yaml`, not reachable by any restore mechanism. Wipe the laptop and they're gone with it.

![66 loose skill directories consolidating into one nix-owned registry](/assets/img/scaling-agentic-workflow.png)
