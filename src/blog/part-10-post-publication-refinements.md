...
One tool in the `ai-tools.nix` module — `code-review-graph` — was pinned to a specific `uv`-managed CPython (3.14) to get matching C headers for a native extension (`watchdog`). It turned out the actual fix for the underlying build failure was unrelated to the Python version: it was the `SDKROOT`/`CPATH` environment variables set earlier in the same module for Xcode SDK headers. Once that was clear, the `--python 3.14` pin was dropped and the tool now just uses uv's default interpreter. One less thing to keep in sync, and the real fix (the SDK env vars) was already in place anyway.

### Font system overhaul

Nerd Font packages (FiraCode, JetBrains Mono, Hack) were originally declared via nix-darwin's `fonts.packages`, but that doesn't reliably register fonts with macOS Core Text on every macOS version. The fix moved them to Homebrew casks (`font-fira-code-nerd-font` etc.), which install directly into `~/Library/Fonts/` — a location macOS always scans. Same fonts, more reliable registration, one less thing that silently doesn't work after a fresh install.
...
This means any agent session history from the last three weeks is lost on machine migration unless manually copied. There is no backup script. There is no restore step. The directories are recreated empty by `creatlinks.sh`.
...
What these three have in common: they are **runtime state**, not configuration. Nix is good at configuration. It has no opinion on runtime state. The mistake was expecting it to.

A `~/.opencode/` directory is not a config file. A `~/.agent-work/` tree is not a package. A `~/.zen/` profile is not a dotfile. Recognising this boundary saves time and prevents the wrong tools from being applied to the wrong problems.

## Status: three-tier agent architecture

Part 9 introduced the three-tier agent design: `@research` (read-only web/Jira), `@plan` (synthesis and spec writing), and `@implement` (code, builds, branches). The design document exists. The threat model is sound. The agent prompts are drafted.
...
The work that replaced it was the post-publication refinements in this post: the fixes, the migration documentation, the corrections. These aren't glamorous, but they make the existing system actually reliable before I add more complexity.
